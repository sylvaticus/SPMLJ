...
urlDataOriginal = "https://fueleconomy.gov/feg/epadata/vehicles.csv"

data = @pipe HTTP.get(urlDataOriginal).body                                                |>
             CSV.File(_, delim=',', missingstring="NA", ignorerepeated=true, header=true) |>
             DataFrame;

cd(@__DIR__)
data = CSV.File("data/vehicles.csv", delim=',', missingstring="NA", ignorerepeated=false, header=true) |> DataFrame
...
X = data[:,[16;18;23;24;25;27;51:53;58;63;64]]
y = data[47]

((xtrain,xval,xtest),(ytrain,yval,ytest)) = partition([Matrix(X),y],[0.7,0.15,0.15],rng=copy(FIXEDRNG))

myTree       = buildTree(xtrain,ytrain, rng=copy(FIXEDRNG), maxDepth = 5, maxFeatures=5)
ŷtrain,ŷval  = predict.(Ref(myTree), [xtrain,xval],rng=copy(FIXEDRNG))

myForest     = buildForest(xtrain,ytrain,30, rng=copy(FIXEDRNG), maxDepth = 20, maxFeatures=6)
ŷtrain,ŷval,ŷtest  = predict.(Ref(myForest), [xtrain,xval,xtest],rng=copy(FIXEDRNG))
...
trainAccuracy,valAccuracy,testAccuracy  = accuracy.([ŷtrain,ŷval,ŷtest],[ytrain,yval,ytest],rng=copy(FIXEDRNG),tol=1)
#src (0.7893390191897655, 0.6730445430889331,0.6768218003674219

cf = ConfusionMatrix(ŷval,yval)
